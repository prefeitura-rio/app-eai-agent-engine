...
# Point to your collector HTTP endpoint (default 4318)
otlp_endpoint = os.getenv(
    "OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4318/v1/traces"
)
otlp_header = os.getenv("OTEL_EXPORTER_OTLP_HEADERS", "")
...
otlp_exporter = OTLPSpanExporter(
    ...
    # Headers can be passed as a dictionary if needed
    headers=(
        dict(header.split("=") for header in otlp_header.split(",") if "=" in header)
        if otlp_header
        else None
    ),
)
...
# Instrument LangChain/LangGraph (do this before creating agents/nodes)
