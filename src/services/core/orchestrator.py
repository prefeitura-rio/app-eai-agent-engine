...
class ServiceOrchestrator:
                        ...
                        if step.payload_schema:
                            if "." in step_path:
                                ...
                                if step.payload_schema.get("properties"):
                                    for field_name in step.payload_schema[
                                        "properties"
                                    ].keys():
    ...
    def execute_turn(
        ...
        if payload:
            ...
            # Process payload data with proper dependency validation
            for step_name, value in payload.items():
                ...
                if step is not None:
                    ...
                    # Only process if dependencies are met OR step is explicitly pending
                    if dependencies_met or step_name in pending_step_names:
                        ...
                        # Log dependency violation for debugging
                        unmet_deps = [
                            dep
                            for dep in step.depends_on
                            if not self._is_dependency_met(
                                dep, state_manager, service_state
                            )
                        ]
                        print(
                            f"⚠️  Dependency violation: {step_name} requires {unmet_deps} to be completed first"
                        )
            ...
            if next_step.action:
                try:
                    ...
                    if result.is_complete:
                        ...
                        # IMPORTANT: Capture execution tree BEFORE applying resets
                        # This preserves the complete execution path for visualization
                    ...
                    error_message = (
                        f"Unexpected error in action '{next_step.action.__name__}': {e}"
                    )
                    break
