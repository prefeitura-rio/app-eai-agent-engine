...
def serialize_source(source: Dict[str, Any]) -> str:
    ...
    Examples:
        >>> serialize_source({"source": "eai-engine", "tool": "search"})
        'source=eai-engine | tool=search'

        >>> serialize_source({"source": "eai-engine", "tool": "multi_step_service", "workflow": "iptu_pagamento"})
        'source=eai-engine | tool=multi_step_service | workflow=iptu_pagamento'

        >>> serialize_source({"source": "eai-engine", "tool": "equipments", "function": "get_nearby", "address": "Av. Vargas"})
        'source=eai-engine | tool=equipments | function=get_nearby | address=Av. Vargas'
    """
    parts = []
...
async def send_api_error(
    ...
    Example - Agent Phase:
        ...
        ...     source={
        ...         "source": "eai-engine",
        ...         "phase": "pre_model_hook",
        ...         "operation": "inject_memory",
        ...         "function": "_inject_long_term_memory"
        ...     },
        ...     api_endpoint="https://mcp-server/get_user_memory",
        ...     request_body={"user_id": "5521999999999"},
        ...     status_code=503,
        ...     error_message="Service Unavailable",
        ... )
        # Flowname: "pre_model_hook(inject_memory)"

    Example - Tool Execution:
        >>> await send_api_error(
        ...     user_id="5521999999999",
        ...     source={"source": "eai-engine", "phase": "tool_execution", "operation": "google_search"},
        ...     api_endpoint="https://mcp-server/tools/google_search",
        ...     request_body={"q": "teste"},
        ...     status_code=500,
        ...     error_message="Internal Error",
        ... )
        # Flowname: "tool_execution(google_search)"
    """

    # Cria flowname simples a partir do source
    # Suporta tanto formato novo (phase/operation) quanto antigo (tool/workflow)
    if "phase" in source:
...
async def send_general_error(
    ...
    Example - Agent Phase:
        ...
        ...     user_id="5521999999999",
        ...     source={"source": "eai-engine", "phase": "agent_node", "operation": "llm_call_async"},
        ...     error_type="TimeoutError",
        ...     error_message="LLM call timed out",
        ... )
        # Flowname: "agent_node(llm_call_async)"
    ...
    Example - Tool Execution:
        ...
        ...         "operation": "multi_step_service(iptu_pagamento)",
        ...         "function": "validate_inscription"
        ...     },
        ...     error_type="ValidationError",
        ...     error_message="Inscrição imobiliária inválida",
        ... )
        # Flowname: "tool_execution(multi_step_service(iptu_pagamento)).validate_inscription"
    """
...
def interceptor(
    ...
    Args:
        source: Dicionário base identificando a origem do erro (formato phase/operation)
    ...
    Example - Agent Phase (Pre-Model Hook):
        >>> @interceptor(
        ...     source={"source": "eai-engine", "phase": "pre_model_hook", "operation": "inject_memory"},
        ...     extract_user_id=lambda args, kwargs: kwargs.get("config", {}).get("configurable", {}).get("thread_id", "unknown")
        ... )
        ... async def _inject_long_term_memory(state, config):
        ...     return await fetch_memory(state, config)
        # Flowname: "pre_model_hook(inject_memory)._inject_long_term_memory"

    Example - Agent Phase (Tool Execution):
        >>> @interceptor(
        ...     source={"source": "eai-engine", "phase": "tool_execution", "operation": "google_search"},
        ...     extract_user_id=lambda args, kwargs: kwargs.get("config", {}).get("configurable", {}).get("thread_id", "unknown")
        ... )
        ... async def execute_google_search(query: str, config):
        ...     return await search_api(query)
        # Flowname: "tool_execution(google_search).execute_google_search"

    Example - Dynamic Source (with context):
        >>> @interceptor(
        ...     source={"source": "eai-engine", "phase": "agent_node", "operation": "llm_call_async"},
        ...     extract_source=lambda args, kwargs, base: {**base, "context": {"model": "gemini-pro"}},
        ...     extract_user_id=lambda args, kwargs: kwargs.get("config", {}).get("configurable", {}).get("thread_id", "unknown")
        ... )
        ... async def acall_model(state, config):
        ...     return await model.ainvoke(state)
        # Flowname: "agent_node(llm_call_async).acall_model"
        # Source includes context: {"model": "gemini-pro"}

    Example - Backward Compatibility (old MCP format still works):
        >>> @interceptor(
        ...     source={"source": "eai-engine", "tool": "multi_step_service", "workflow": "iptu_pagamento"},
        ...
        # Flowname: "multi_step_service(iptu_pagamento).consultar_guias"
    """
