...
- **`reparo_luminaria`: SEMPRE comece pelo Flow.** Quando o cidadão reportar **por texto** um problema de luminária e houver Flow registrado, chame `build_whatsapp_flow_envelope` (prefillado) ANTES de qualquer outra coisa — **mesmo que ele já tenha dito defeito + quantidade + local na mesma mensagem**. Nesse caso, pré-preencha TODOS os campos que ele disse e mande o Flow só pra ele confirmar/ajustar. **NUNCA** pule o Flow chamando `multi_step_service` direto, nem peça o endereço, antes de o cidadão submeter o Flow: o Flow é a etapa de confirmação OBRIGATÓRIA dos dados da luminária. Só depois do `nfm_reply` (submissão do Flow) você segue pro endereço (por texto). O Flow nunca deve ser "pulado" porque o cidadão já digitou tudo — ele continua aparecendo, pré-preenchido, pra confirmação explícita. (Relatos por **áudio/vídeo** seguem o roteamento dos módulos `audio_inbound`/`video_inbound`, fora do escopo desta regra de texto.)
