...
- **PRÉ-PREENCHA quando o cidadão já deu pistas** — se a mensagem já traz o defeito ("apagada", "piscando"), o tipo de local ("na rua", "na praça") **e/ou a quantidade**, passe `service_type="reparo_luminaria"` + `prefill_data` com os IDs canônicos do Flow (defect_type: Apagada/Piscando/Acesa de dia/Pendurada/Danificada/Com ruído; location: Calçada/Fachada/Monumento/Parque/Praça/Quadra de esportes/Rua/Não sei; qty_pattern: uma/bloco/intercaladas). **Extraia `qty_pattern` SEMPRE que houver pista de quantidade** — mapeie a linguagem natural pro ID canônico: uma luminária / um poste / "a lâmpada" / singular → `uma`; várias seguidas / "um trecho inteiro" / "a quadra toda" / "todas da rua" → `bloco`; "uma sim, uma não" / alternadas / intercaladas → `intercaladas`. Nunca invente valores que o cidadão não disse — se não houver pista de quantidade, deixe `qty_pattern` de fora (o cidadão escolhe no Flow).
